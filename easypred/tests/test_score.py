...
def test_unique_score(score, decimals, excpected):
    score.computation_decimals = decimals
    np.testing.assert_array_equal(score.unique_scores, excpected)
    # Restore default decimals
    score.computation_decimals = 3


@pytest.mark.parametrize(
    "score, threshold, excpected",
    [
...
def test_score_to_values(score, threshold, excpected):
    result = score.score_to_values(threshold=threshold)
    np.testing.assert_array_equal(result, excpected)
...
def test_accuracy_scores(score, decimals, excpected):
    score.computation_decimals = decimals
    np.testing.assert_array_equal(score.accuracy_scores, excpected)
...
def test_false_positive_rates(score, decimals, excpected):
    score.computation_decimals = decimals
    np.testing.assert_array_equal(score.false_positive_rates, excpected)
